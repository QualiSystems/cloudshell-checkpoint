...
SCP_COPY = CommandTemplate(
    command="scp -P {scp_port} {src_location} {dst_location}",
    error_map=SCP_ERROR_MAP

)
FTP_UPLOAD = CommandTemplate(
        command="put {src_location} {dst_location}",
        error_map=FTP_ERROR_MAP
    )
FTP_DOWNLOAD = CommandTemplate(
    command="get {src_location} {dst_location}",
    error_map=FTP_ERROR_MAP
)
...
SHUTDOWN = CommandTemplate(
    ...
    action_map={
        r"system is going down": lambda: (_ for _ in ()).throw(ShutdownOkCheckpointError())
    }
)
