...
class SnmpIfEntity(object):
    ...
    def _ip_map(self):
        ...
        for ip_addr, index_obj in self._port_attributes_snmp_tables.ip_mixed_dict.items():
    ...
    def ipv4_address(self):
        for ip_addr in self._ip_map.get("ipv4", []):
            try:
                return str(IPv4Address(binascii.unhexlify(re.sub(r"ipv4.*?\.0x", "", ip_addr))[:4]))
            except:
                continue

        if self._port_attributes_snmp_tables.ip_v4_old_dict:
            for ip_addr, index in self._port_attributes_snmp_tables.ip_v4_old_dict.items():
    ...
    def ipv6_address(self):
        for ip_addr in self._ip_map.get('ipv6', []):
            try:
                return str(IPv6Address(binascii.unhexlify(re.sub(r"ipv6.*?\.0x", "", ip_addr))[:16]))
            except:
                continue
