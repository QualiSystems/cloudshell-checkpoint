...
class SnmpIfPort(SnmpIfEntity):
    ...
    def if_type(self):
        if not self._if_type:
            self._if_type = "other"
            if_type = self._snmp.get_property(SnmpMibObject(*(port_constants.PORT_TYPE + (self.if_index,)))).safe_value
    ...
    def if_speed(self):
        if not self._if_speed:
            self._if_speed = self._snmp.get_property(
                SnmpMibObject(*(port_constants.PORT_SPEED + (self.if_index,)))).safe_value
    ...
    def if_mtu(self):
        if not self._if_mtu:
            self._if_mtu = self._snmp.get_property(
                SnmpMibObject(*(port_constants.PORT_MTU + (self.if_index,)))).safe_value
        return self._if_mtu
    ...
    def if_mac(self):
        if not self._if_mac:
            self._if_mac = self._snmp.get_property(
                SnmpMibObject(*(port_constants.PORT_MAC + (self.if_index,)))).safe_value
        return self._if_mac
                ...
                if key:
                    for port_id, rem_table in self._port_attributes_snmp_tables.lldp_remote_table.items():
                        if key in port_id.split("."):
                            ...
                            remote_port_name = self._snmp.get_property(SnmpMibObject(**(
                                    port_constants.PORT_ADJACENT_REM_PORT_DESCR + (port_id,)))).safe_value
    ...
    def _get_auto_neg(self):
        ...
        auto_negotiation = self._snmp.get_property(SnmpMibObject(*(
                port_constants.PORT_AUTO_NEG + (index,)))).safe_value
        ...
        for key, value in self._port_attributes_snmp_tables.duplex_table.items():
            if 'dot3StatsIndex' in value.keys() and value['dot3StatsIndex'] == str(self.if_index):
                interface_duplex = self._snmp.get_property(
                    SnmpMibObject('EtherLike-MIB', 'dot3StatsDuplexStatus', key)).safe_value
